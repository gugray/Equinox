...
function doShit() {
  ...
  const flowFun = (pt, res) => {
    ...
    res.y = vec[1];
    res.len = null;
    if (res.length() < 0.0005) return false;
    res.normalize();
    return true;
  };
  const densityFun = (pt) => {
  ...
  const flopt = {
    ...
    minCellSize: 2, maxCellSize: 64, nShades: 16, logGrid: true
  }

  const flgen = new FlowLineGenerator(flopt);
  while (true) {
    const flLength = flgen.genFlowLine();
    if (flLength == -1) break;
  }
  ...
  // }
  let start = true;
  for (let ix = 0; ix < flgen.vertIx; ix += 2) {
    if (isNaN(flgen.verts[ix])) {
      start = true;
      continue;
    }
    if (start) {
      ctx.moveTo(flgen.verts[ix], h - flgen.verts[ix + 1]);
      start = false;
    }
    else ctx.lineTo(flgen.verts[ix], h - flgen.verts[ix + 1]);
  }
  ctx.stroke();


  // ctx.fillStyle = "red";
  // ctx.fillRect(425, 300, 5, 5);
